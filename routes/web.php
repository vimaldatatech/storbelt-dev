...
use App\Http\Controllers\staff\UserList;
...
use App\Http\Controllers\storman\StormanSyncController;
...
Route::middleware('auth')->group(function () {
    ...
    Route::get('/app/user/view/account/{user}', [UserList::class, 'viewUser'])->name('app-user-view-account');
    Route::get('/users/data', [UserList::class, 'list'])->name('users.data');
    Route::get('/users/{user}', [UserList::class, 'show'])->name('users.show'); // for edit fetch
    Route::post('/users', [UserList::class, 'store'])->name('users.store');
    Route::put('/users/{user}', [UserList::class, 'update'])->name('users.update');
    Route::delete('/users/{user}', [UserList::class, 'destroy'])->name('users.destroy');

    // User permissions routes
    Route::get('/users/{user}/permissions', [UserList::class, 'permissions']);
    Route::post('/users/{user}/permissions', [UserList::class, 'syncPermissions']);

    // Company Module Routes
    Route::get('/app/company/list', [CompanyController::class, 'index'])->name('company');
    Route::get('/company/data', [CompanyController::class, 'list'])->name('company.data');

    Route::resource('/company-list', CompanyController::class);


    // Permission Routes
    Route::get('/permission/meta', [PermissionController::class, 'meta']);
    ...
    // Storman routes
    Route::get('/storman-settings', [StormanSyncController::class, 'index'])->name('storman.sync');
    Route::put('/storman/save', [StormanSyncController::class, 'save'])->name('storman.save');
    Route::post('/storman-sync', [StormanSyncController::class, 'sync'])->name('storman.sync.post');
